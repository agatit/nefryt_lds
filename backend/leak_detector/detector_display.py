...
class DetectorDisplay:
    ...
    def set_params(self, data_start: np.ndarray, data_end: np.ndarray, dp_indexes_list: list,
    ...
    def display(self, segment_number: int, plot: bool, traces: list) -> None:
        step_xtick_ms = 1000
        step_ytick_m = 50
        ...
        logging.info(f'Method ID={method.id}: '
                     ...
                     f'{xticks_labels[segment.max_window_size // 10 + int(max_x)]}.{((int(max_y)) % 100) * 10:03d}) '
        ...
        if plot:
            ...
            ax2 = sns.heatmap(probability.T, ax=ax2, cbar_ax=ax_cbar, vmin=0, vmax=0.13)
