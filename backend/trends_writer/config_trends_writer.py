...
def set_manager_handler(default_manager_handler: dict) -> dict:
    ...
    if platform.system() == "Windows":
        ...
        default_manager_handler_filename = path / default_manager_handler.pop(
            "filename", "default_log.log"
        )
    elif platform.system() in ["Darwin", "Linux"]:
        default_manager_handler["class"] = "logging.handlers.SysLogHandler"
        default_manager_handler["address"] = (
...
_logging_config["handlers"]["manager"] = set_manager_handler(
    _logging_config["handlers"]["manager"]
)
...
try:
    ...
    if multiprocessing.current_process().name == "MainProcess":
        logging.warning(
            f"TrendsWriter config: Cannot configurate logger with given parameters: {e}",
            exc_info=True,
        )
    _logging_config["handlers"]["manager"] = reset_manager_handler(
        _logging_config["handlers"]["manager"]
    )
    logging.config.dictConfig(_logging_config)
