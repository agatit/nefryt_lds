...
def test_trend_deriv_calculates_mean_correctly_for_constant_trend():
...
def test_trend_deriv_calculates_mean_correctly_for_almost_constant_trend():
    filter_window_value = 1
    value = 250
...
def test_trend_mean_calculates_mean_correctly_for_changing_monotonic_trend():
    ...
    for i, (saved_data, saved_timestamp) in enumerate(saved_results):
        if i < len(saved_results) // 2:
            start = (i + filter_window_value) * 100
            end = (i + 1 + filter_window_value) * 100
            step = 1
            assert np.all(np.diff(np.array(saved_data)) >= 0)
        else:
            start = (6 - i - filter_window_value) * 100 - 1
            end = (5 - i - filter_window_value) * 100 - 1
            step = -1
            assert np.all(np.diff(np.array(saved_data)) <= 0)
        assert np.all(np.arange(start, end, step) - np.array(saved_data) >= 0)
        assert saved_timestamp == i + filter_window_value + timestamp_offset
