...
from ..schemas import Error, SimulationDefBase, SimulationBase, Information, UpdateSimulation

# TODO: uzupełnić openapi
# TODO: CRUD simulation params
# TODO: alembic
...
async def list_simulation_defs(engine: Annotated[Engine, Depends(get_engine)], params: Annotated[CustomParams, Depends()],
    ...
    try:
        ...
        error = Error(code=status.HTTP_500_INTERNAL_SERVER_ERROR, message='Exception in list_simulation_defs(): ' + str(e))
...
async def create_simulation(simulation: Annotated[SimulationBase, Body()], engine: Annotated[Engine, Depends(get_engine)]):
    try:
...
async def delete_simulation_by_id(simulation_id: Annotated[int, Path()], engine: Annotated[Engine, Depends(get_engine)]):
    try:
        # TODO: sprawdzić czy usuwają się SimulationParam
        with Session(engine) as session:
...
async def get_simulation_by_id(simulation_id: int, engine: Annotated[Engine, Depends(get_engine)]):
    try:
        ...
        error = Error(code=status.HTTP_500_INTERNAL_SERVER_ERROR, message='Exception in get_simulation_by_id(): ' + str(e))
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)


@router.put('/{simulation_id}', response_model=lds.Simulation | Error)
async def update_simulation(simulation_id: Annotated[int, Path()], updated_simulation: Annotated[UpdateSimulation, Body()],
    ...
    try:
        ...
        error = Error(code=status.HTTP_409_CONFLICT, message='Integrity error when updating simulation with id = ' + str(simulation_id))
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_409_CONFLICT)
    except Exception as e:
