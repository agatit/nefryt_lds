...
@router.get('', response_model=CustomPage[lds.Link] | api.Error)
async def list_links(engine: Annotated[Engine, Depends(get_engine)], params: Annotated[CustomParams, Depends()],
    ...
    try:
        ...
        error = api.Error(code=status.HTTP_500_INTERNAL_SERVER_ERROR, message='Exception in list_links(): ' + str(e))
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)


@router.post('', response_model=lds.Link | api.Error)
async def create_link(link: Annotated[api.LinkCreate, Body()], engine: Annotated[Engine, Depends(get_engine)]):
    try:
        ...
        error = api.Error(code=status.HTTP_409_CONFLICT, message='Integrity error when creating link')
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_409_CONFLICT)
    except Exception as e:
        error = api.Error(code=status.HTTP_500_INTERNAL_SERVER_ERROR, message='Exception in create_link(): ' + str(e))
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)


@router.delete('/{link_id}', response_model=None | api.Error)
async def delete_link_by_id(link_id: Annotated[int, Path()], engine: Annotated[Engine, Depends(get_engine)]):
    try:
        ...
        error = api.Error(code=status.HTTP_500_INTERNAL_SERVER_ERROR, message='Exception in delete_link_by_id(): ' + str(e))
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)


@router.get('/{link_id}', response_model=lds.Link | api.Error)
async def get_link_by_id(link_id: Annotated[int, Path()], engine: Annotated[Engine, Depends(get_engine)]):
    try:
        with Session(engine) as session:
            link = session.get(lds.Link, link_id)
        if not link:
            error = api.Error(code=status.HTTP_404_NOT_FOUND, message='No link with id = ' + str(link_id))
            return JSONResponse(content=error.model_dump(), status_code=status.HTTP_404_NOT_FOUND)
        return link
    except Exception as e:
        error = api.Error(code=status.HTTP_500_INTERNAL_SERVER_ERROR, message='Exception in get_link_by_id(): ' + str(e))
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)


@router.put('/{link_id}', response_model=lds.Link | api.Error)
async def update_link(link_id: Annotated[int, Path()], updated_link: Annotated[api.LinkUpdate, Body()],
    ...
    try:
        ...
        error = api.Error(code=status.HTTP_500_INTERNAL_SERVER_ERROR, message='Exception in update_link(): ' + str(e))
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
