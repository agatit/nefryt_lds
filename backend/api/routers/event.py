...
@router.get('', response_model=CustomPage[api.Event] | api.Error)
async def list_events(engine: Annotated[Engine, Depends(get_engine)], params: Annotated[CustomParams, Depends()],
    ...
    try:
        ...
        error = api.Error(code=status.HTTP_500_INTERNAL_SERVER_ERROR, message='Exception in list_events(): ' + str(e))
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)


@router.get('/{event_id}', response_model=api.Event | api.Error)
...
@router.post('/{event_id}/ack', response_model=api.Information | api.Error)
async def ack_event(event_id: int, permissions: Annotated[list[str], Depends(get_user_permissions)],
                    engine: Annotated[Engine, Depends(get_engine)]):
    if 'admin' not in permissions:
        error = api.Error(code=status.HTTP_403_FORBIDDEN, message='Action requires admin permissions')
    ...
    try:
        with Session(engine) as session:
            ...
            session.add(event)
            session.commit()
        return api.Information(message="Event acknowledged", affected=1, status=status.HTTP_200_OK)
    except NoResultFound:
        error = api.Error(code=status.HTTP_404_NOT_FOUND, message='No event with id = ' + str(event_id))
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_404_NOT_FOUND)
    except Exception as e:
        error = api.Error(code=status.HTTP_500_INTERNAL_SERVER_ERROR, message='Exception in ack_event(): ' + str(e))
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
