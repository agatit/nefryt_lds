...
def test_list_units_should_return_ok_response_code_and_empty_list_when_no_units(test_client):
    response = test_client.get("/unit")
    assert response.status_code == status.HTTP_200_OK
    assert len(response.json()['items']) == 0


@pytest.mark.parametrize('add_test_context', [reset_unit_objects], indirect=True)
@pytest.mark.usefixtures("add_test_context")
def test_list_units_should_return_ok_response_code_and_correct_units(test_client):
    response = test_client.get("/unit")
    assert response.status_code == status.HTTP_200_OK
...
@pytest.mark.parametrize('add_test_context', [reset_unit_objects], indirect=True)
@pytest.mark.usefixtures("add_test_context")
def test_list_units_should_return_ok_response_code_and_correct_page_data(test_client):
    size = 2
    page = 1
...
@pytest.mark.parametrize('add_test_context', [reset_unit_objects], indirect=True)
@pytest.mark.usefixtures("add_test_context")
def test_list_units_should_return_ok_response_code_and_default_page_data(test_client):
    response = test_client.get("/unit")
    assert response.status_code == status.HTTP_200_OK
    assert len(response.json()) == 5
...
@pytest.mark.parametrize('add_test_context', [reset_unit_objects], indirect=True)
@pytest.mark.usefixtures("add_test_context")
def test_list_units_should_return_ok_response_code_and_data_filtered_by_odata_query(test_client):
...
@pytest.mark.parametrize('add_test_context', [reset_unit_objects], indirect=True)
@pytest.mark.usefixtures("add_test_context")
def test_create_unit_should_return_created_response_code_and_created_unit_data(test_client):
...
@pytest.mark.parametrize('add_test_context', [reset_unit_objects], indirect=True)
@pytest.mark.usefixtures("add_test_context")
def test_delete_unit_by_id_should_return_no_content_response_code_and_remove_unit(test_client):
...
def test_delete_unit_by_id_should_return_not_found_response_code_and_error_when_no_unit_with_given_id(test_client):
    response = test_client.delete("/unit/" + unit1.ID)
...
@pytest.mark.parametrize('add_test_context', [reset_unit_objects], indirect=True)
@pytest.mark.usefixtures("add_test_context")
def test_get_unit_by_id_should_return_ok_response_code_and_unit_of_given_id(test_client):
...
def test_get_unit_by_id_should_return_not_found_response_code_and_error_when_no_unit_with_given_id(test_client):
...
@pytest.mark.parametrize('add_test_context', [reset_unit_objects], indirect=True)
@pytest.mark.usefixtures("add_test_context")
def test_update_unit_should_return_ok_response_code_and_unit_of_given_id(test_client):
...
def test_update_unit_should_return_not_found_response_code_and_error_when_no_unit_with_given_id(test_client):
    updated_unit_dict = {'Symbol': 'Fa'}
