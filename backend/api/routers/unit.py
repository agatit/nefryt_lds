...
@router.get('', response_model=CustomPage[lds.Unit] | api.Error)
...
@router.post('', response_model=lds.Unit | api.Error)
async def create_unit(unit: Annotated[api.UnitCreate, Body()], engine: Annotated[Engine, Depends(get_engine)]):
    try:
        ...
        error = api.Error(code=status.HTTP_409_CONFLICT, message='Integrity error when creating unit')
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_409_CONFLICT)
    except Exception as e:
        error = api.Error(code=status.HTTP_500_INTERNAL_SERVER_ERROR, message='Exception in create_unit(): ' + str(e))
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)


@router.delete('/{unit_id}', response_model=None | api.Error)
async def delete_unit_by_id(unit_id: Annotated[str, Path()], engine: Annotated[Engine, Depends(get_engine)]):
    try:
        with Session(engine) as session:
            ...
            if not unit:
                error = api.Error(code=status.HTTP_404_NOT_FOUND, message='No unit with id = ' + unit_id)
                return JSONResponse(content=error.model_dump(), status_code=status.HTTP_404_NOT_FOUND)
            session.delete(unit)
            session.commit()
        return Response(status_code=status.HTTP_204_NO_CONTENT)
    except Exception as e:
        error = api.Error(code=status.HTTP_500_INTERNAL_SERVER_ERROR, message='Exception in delete_unit_by_id(): ' + str(e))
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)


@router.get('/{unit_id}', response_model=lds.Unit | api.Error)
...
@router.put('/{unit_id}', response_model=lds.Unit | api.Error)
async def update_unit(unit_id: Annotated[str, Path()], updated_unit: Annotated[api.UnitUpdate, Body()],
    ...
    try:
        ...
        error = api.Error(code=status.HTTP_500_INTERNAL_SERVER_ERROR, message='Exception in update_unit(): ' + str(e))
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
