...
async def list_profiler_data(engine: Annotated[Engine, Depends(get_engine)],
    ...
    try:
        ...
        error = Error(code=status.HTTP_500_INTERNAL_SERVER_ERROR, message='Exception in list_profiler_data(): ' + str(e))
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)


@router.get('/general', response_model=ProfilerGeneralData | Error)
async def get_general_profiler_data(engine: Annotated[Engine, Depends(get_engine)]):
    try:
        ...
        general_data = ProfilerGeneralData()
        if len(profiler_datas) == 0:
            error = Error(code=status.HTTP_400_BAD_REQUEST, message='No stored profiler data to calculate general data')
        ...
        error = Error(code=status.HTTP_500_INTERNAL_SERVER_ERROR, message='Exception in get_general_profiler_data(): ' + str(e))
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
async def get_profiler_data_by_id(trend_id: Annotated[int, Path()], engine: Annotated[Engine, Depends(get_engine)]):
    try:
        ...
        if not profiler_data:
            error = Error(code=status.HTTP_404_NOT_FOUND, message='No profiler data for trend with id = ' + str(trend_id))
            return JSONResponse(content=error.model_dump(), status_code=status.HTTP_404_NOT_FOUND)
        return profiler_data
    except Exception as e:
        error = Error(code=status.HTTP_500_INTERNAL_SERVER_ERROR, message='Exception in get_profiler_data_by_id(): ' + str(e))
        return JSONResponse(content=error.model_dump(), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
